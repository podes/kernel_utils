 ...
 */
void *
dlm_avl_walk(avl_tree_t *tree, void	*oldnode, int left)
{
 ...
 */
void *
dlm_avl_nearest(avl_tree_t *tree, avl_index_t where, int direction)
{
	...
	return (dlm_avl_walk(tree, data, direction));
}
 ...
 */
void *
dlm_avl_find(avl_tree_t *tree, const void *value, avl_index_t *where)
{
	avl_node_t *node;
	avl_node_t *prev = NULL;
	int child = 0;
 ...
 */
void
dlm_avl_insert(avl_tree_t *tree, void *new_data, avl_index_t where)
{
	avl_node_t *node;
	...
	int direction)
{
    ...
    dlm_avl_insert(tree, new_data, AVL_MKINDEX(node, child));
}
 ...
 */
void
dlm_avl_add(avl_tree_t *tree, void *new_node)
{
	avl_index_t where = 0;
	 ...
	 */
	if (dlm_avl_find(tree, new_node, &where) != NULL)
#ifdef _KERNEL
		panic("avl_find() succeeded inside avl_add()");
#else
	assert(0);
#endif
    dlm_avl_insert(tree, new_node, where);
}
 ...
 */
void
dlm_avl_remove(avl_tree_t *tree, void *data)
{
	avl_node_t *delete;
	avl_node_t *parent;
	avl_node_t *node;
...
#define	AVL_REINSERT(tree, obj)			\
	dlm_avl_remove((tree), (obj));		\
    dlm_avl_add((tree), (obj))

boolean_t
dlm_avl_update_lt(avl_tree_t *t, void *obj)
{
	void *neighbor;
...
boolean_t
dlm_avl_update_gt(avl_tree_t *t, void *obj)
{
	void *neighbor;
...
boolean_t
dlm_avl_update(avl_tree_t *t, void *obj)
{
	void *neighbor;
 ...
 */
void
dlm_avl_create(avl_tree_t *tree, int (*compar) (const void *, const void *),
...
/* ARGSUSED */
void
dlm_avl_destroy(avl_tree_t *tree)
{
	assert(tree);
	assert(tree->avl_numnodes == 0);
	assert(tree->avl_root == NULL);
 ...
 */
ulong_t
dlm_avl_numnodes(avl_tree_t *tree)
{
	assert(tree);
...
boolean_t
dlm_avl_is_empty(avl_tree_t *tree)
{
	assert(tree);
 ...
 */
void *
dlm_avl_destroy_nodes(avl_tree_t *tree, void **cookie)
{
	avl_node_t	*node;
	avl_node_t	*parent;
	int		child;
